...
import micasense.metadata_custom as metadata
...
def ret_micasense_pos(mrk_folder, micasense_folder, image_suffix, epsg_crs, out_file, P1_shift_vec):
        ...
        # Use micasense.metadata to get the correct UTC time
            image_metadata = metadata.Metadata(file)
            utc_time = image_metadata.utc_time()
            mica_timestamp = datetime(utc_time) #- datetime.timedelta(seconds=MICA_deltat)
