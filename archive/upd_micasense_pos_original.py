...
def calculate_average_interval(timestamps):
    ...
    if intervals:
        ...
        return intervals, avg_interval, std_dev
    return [], 0, 0
...
def ret_micasense_pos(mrk_folder, micasense_folder, image_suffix, epsg_crs, out_file, P1_shift_vec):
    ...
    # Filter out MicaSense images taken outside of the P1 timeframe
    filtered_mica_events = [timestamp for timestamp in mica_events if first_P1_timestamp <= timestamp.timestamp() <= last_P1_timestamp]
    
    # Sort MicaSense images by timestamp
    sorted_mica_events = sorted(filtered_mica_events)
    
    # Calculate and print the average time interval and standard deviation between MicaSense images
    intervals, avg_interval, std_dev = calculate_average_interval(sorted_mica_events)
    print(f"Average time interval between MicaSense images: {avg_interval} seconds")
    print(f"Standard deviation of time intervals: {std_dev} seconds")
    
    # Create a CSV file with sorted MicaSense timestamps and intervals
    sorted_timestamps_file = os.path.join(os.path.dirname(out_file), "sorted_micasense_timestamps.csv")
    with open(sorted_timestamps_file, 'w') as f:
        f.write("Timestamp,Interval\n")
        for i, timestamp in enumerate(sorted_mica_events):
            interval = intervals[i-1] if i > 0 else ""
            f.write(f"{timestamp},{interval}\n")
    print(f"Sorted MicaSense timestamps and intervals written to {sorted_timestamps_file}")
    
    # Print sorted MicaSense image names with timestamps
    sorted_image_names = [(filelist[mica_events.index(timestamp)].split("\\")[-1], timestamp) for timestamp in sorted_mica_events]
    print("Sorted MicaSense image names with timestamps:")
    for image_name, timestamp in sorted_image_names:
        print(f"{image_name}: {timestamp}")
    ...
    for m_cam_time in mica_events:
        ...
        if P1_triggered:    
            if P1_pos_time <= camera_time_sec:
                ...
                upd_pos1 = P1_pos[a]
                upd_pos2 = P1_pos[a+1]
                print(f"Adjusting MicaSense image at {m_cam_time} using P1 images at {P1_events[a]} and {P1_events[a+1]}")
            elif P1_pos_time > camera_time_sec:
                time1 = P1_events[a-1].timestamp()
                time2 = P1_pos_time
                upd_pos1 = P1_pos[a-1]
                upd_pos2 = P1_pos[a]
                print(f"Adjusting MicaSense image at {m_cam_time} using P1 images at {P1_events[a-1]} and {P1_events[a]}")
